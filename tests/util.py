...
class TempDirTestCaseBase(GetPathTestCase):
    ...
    def CheckCallWithPath(self, cmd, expected_retcode=0, stdin_str=""):
        ...
        cmd = [
            ...
            "exec(open('%s'))" % (transitfeed_parent, script_path, script_path),
        ] + script_args
        return check_call(
            cmd,
...
class ValidationTestCase(TestCase):
    ...
    def ValidateAndExpectInvalidValue(self, object, column_name, value=INVALID_VALUE):
        self.accumulator.AssertNoMoreExceptions()
        object.Validate(self.problems)
        self.ExpectException("InvalidValue", column_name, value)

    def ExpectInvalidValueInClosure(self, column_name, value=INVALID_VALUE, c=None):
        self.accumulator.AssertNoMoreExceptions()
        rv = c()
        self.ExpectException("InvalidValue", column_name, value)
...
class RecordingProblemAccumulator(transitfeed.ProblemAccumulatorInterface):
    ...
    def PopInvalidValue(self, column_name, file_name=None):
        return self.PopColumnSpecificException("InvalidValue", column_name, file_name)

    def PopMissingValue(self, column_name, file_name=None):
        return self.PopColumnSpecificException("MissingValue", column_name, file_name)
