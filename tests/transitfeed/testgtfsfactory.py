...
class TestGtfsFactory(util.TestCase):
    ...
    def testCanUpdateMapping(self):
        ...
        self.assertEqual(
            self._factory.GetGtfsClassByFileName("agency.txt"), transitfeed.Stop
        )
        self.assertFalse(self._factory.IsFileRequired("agency.txt"))
    ...
    def testCanAddMapping(self):
        self._factory.AddMapping(
            "newrequiredfile.txt",
            {"required": True, "classes": ["NewRequiredClass"], "loading_order": -20},
        )
    ...
    def testCanRemoveMapping(self):
        ...
        self.assertEqual(self._factory.GetGtfsClassByFileName("agency.txt"), None)
        self.assertFalse(self._factory.IsFileRequired("agency.txt"))
    ...
    def testFactoryReturnsClassesAndNotInstances(self):
        for filename in (
            ...
            self.assertTrue(
                ...
                "classes and not instances. This is not the case for " + filename,
            )
    ...
    def testCanFindClassByFileName(self):
        self.assertEqual(
            transitfeed.Agency, self._factory.GetGtfsClassByFileName("agency.txt")
        )
        self.assertEqual(
        ...
        self.assertEqual(
            transitfeed.FareRule, self._factory.GetGtfsClassByFileName("fare_rules.txt")
        )
        self.assertEqual(
        ...
        self.assertEqual(
            transitfeed.Route, self._factory.GetGtfsClassByFileName("routes.txt")
        )
        self.assertEqual(
        ...
        self.assertEqual(
            transitfeed.StopTime, self._factory.GetGtfsClassByFileName("stop_times.txt")
        )
        self.assertEqual(
            transitfeed.Transfer, self._factory.GetGtfsClassByFileName("transfers.txt")
        )
        self.assertEqual(
...
class TestGtfsFactoryUser(util.TestCase):
    def AssertDefaultFactoryIsReturnedIfNoneIsSet(self, instance):
        self.assertTrue(isinstance(instance.GetGtfsFactory(), transitfeed.GtfsFactory))
