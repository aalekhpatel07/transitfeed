...
class ScheduleBuilderTestCase(util.TempFileTestCaseBase):
    ...
    def testBuildFeedWithUtf8Names(self):
        ...
        stop2 = schedule.AddStop(lng=140.001, lat=48.201, name="remote \u020b station")
    ...
    def testBuildSimpleFeed(self):
        ...
        schedule.AddAgency("Test Agency", "http://example.com", "America/Los_Angeles")
    ...
    def testStopIdConflict(self):
        ...
        self.assertEqual(
            "stop0 stop1 stop2 stop4 stop5", " ".join([s.stop_name for s in stop_list])
        )
    ...
    def testRouteIdConflict(self):
        ...
        self.assertEqual("0 1 3 4 R", " ".join(r.route_short_name for r in route_list))
        self.assertMatchesRegex(
            "0 1 3 4 \d{7,9}", " ".join(r.route_id for r in route_list)
        )
        self.assertEqual(
            "Long Name,,,GooCar,LNR", ",".join(r.route_long_name for r in route_list)
        )
    ...
    def testTripIdConflict(self):
        ...
        self.assertEqual(",hs1,hs2,,", ",".join(t["trip_headsign"] for t in trip_list))
        for t in trip_list:
...
class WriteSampleFeedTestCase(util.TempFileTestCaseBase):
    ...
    def runTest(self):
        ...
        stop_time_data = {
            ...
            "AAMV3": [
                ("13:00:00", "13:00:00", "BEATTY_AIRPORT", None, None, None, None),
        ...
        for trip_id in headway_trips:
            headway_trips[trip_id] = schedule.GetTrip(trip_id).GetFrequencyTuples()
        ...
        self.assertEqual(
            weekend_period, read_schedule.GetServicePeriod(weekend_period.service_id)
        )
        ...
        self.assertEqual(
            feed_info.feed_publisher_name, read_schedule.feed_info.feed_publisher_name
        )
        self.assertEqual(
