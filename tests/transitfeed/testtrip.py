...
class DuplicateStopSequenceTestCase(util.TestCase):
    def runTest(self):
        ...
        schedule.Load(util.DataPath("duplicate_stop_sequence"), extra_validation=True)
...
class MissingEndpointTimesTestCase(util.TestCase):
    def runTest(self):
        ...
        problems = transitfeed.ProblemReporter(accumulator)
        schedule = transitfeed.Schedule(problem_reporter=problems)
        schedule.Load(util.DataPath("missing_endpoint_times"), extra_validation=True)
...
class TripSequenceValidationTestCase(util.ValidationTestCase):
    def runTest(self):
        ...
        stoptime1 = transitfeed.StopTime(
            self.problems, stop1, stop_time="12:00:00", stop_sequence=1
        )
        stoptime2 = transitfeed.StopTime(
            self.problems, stop2, stop_time="11:30:00", stop_sequence=2
        )
        stoptime3 = transitfeed.StopTime(
            self.problems, stop3, stop_time="12:15:00", stop_sequence=3
        )
...
class TripServiceIDValidationTestCase(util.ValidationTestCase):
    def runTest(self):
        ...
        self.ExpectInvalidValueInClosure(
            ...
            value="WEEKDAY",
            c=lambda: schedule.AddTripObject(trip1, validate=True),
        )
...
class ShapeDistTraveledOfStopTimeValidationTestCase(util.ValidationTestCase):
    def runTest(self):
        ...
        trip.AddStopTime(
            stop,
            arrival_time="5:18:00",
            departure_time="5:19:00",
            stop_sequence=2,
...
class TripAddStopTimeObjectTestCase(util.ValidationTestCase):
    def runTest(self):
        ...
        schedule.AddAgency(
            "\xc8\x8b Fly Agency", "http://iflyagency.com", "America/Los_Angeles"
        )
        service_period = schedule.GetDefaultServicePeriod().SetDateHasService(
            "20070101"
        )
        ...
        trip.AddStopTimeObject(
            transitfeed.StopTime(
                self.problems, stop1, arrival_secs=10, departure_secs=10
            ),
            schedule=schedule,
        ...
        trip.AddStopTimeObject(
            transitfeed.StopTime(
                self.problems, stop2, arrival_secs=20, departure_secs=20
            ),
            schedule=schedule,
            problems=self.problems,
        )
        ...
        trip.AddStopTimeObject(
            transitfeed.StopTime(self.problems, stop1),
            schedule=schedule,
            problems=self.problems,
        )
        self.ExpectOtherProblemInClosure(
        ...
        trip.AddStopTimeObject(
            transitfeed.StopTime(
                self.problems, stop1, arrival_secs=30, departure_secs=30
            ),
            schedule=schedule,
            problems=self.problems,
        )
...
class TripReplaceStopTimeObjectTestCase(util.TestCase):
    def runTest(self):
        ...
        trip = route.AddTrip(schedule, "bus trip")
        stoptime = transitfeed.StopTime(
            transitfeed.default_problem_reporter,
            stop1,
            arrival_secs=10,
            departure_secs=10,
        )
...
class SingleTripTestCase(util.TestCase):
    def setUp(self):
        ...
        route = schedule.AddRoute(
            short_name="54C", long_name="Polish Hill", route_type=3
        )
...
class TripStopTimeAccessorsTestCase(SingleTripTestCase):
    def runTest(self):
        ...
        self.assertEqual(
            transitfeed.FormatSecondsSinceMidnight(self.trip.GetStartTime()), "05:11:00"
        )
        self.assertEqual(
            transitfeed.FormatSecondsSinceMidnight(self.trip.GetEndTime()), "05:16:00"
        )

        self.trip.AddStopTime(self.stop1, stop_time="05:20:00")
        self.assertEqual(
            transitfeed.FormatSecondsSinceMidnight(self.trip.GetEndTime()), "05:20:00"
        )

        self.trip.AddStopTime(self.stop2, stop_time="05:22:00")
        self.assertEqual(
            transitfeed.FormatSecondsSinceMidnight(self.trip.GetEndTime()), "05:22:00"
        )
...
class TripClearStopTimesTestCase(util.TestCase):
    def runTest(self):
        ...
        schedule.NewDefaultAgency(
            agency_name="Test Agency", agency_timezone="America/Los_Angeles"
        )
...
class AddFrequencyValidationTestCase(util.ValidationTestCase):
    def ExpectInvalidValue(self, start_time, end_time, headway, column_name, value):
        try:
            trip = transitfeed.Trip()
    ...
    def runTest(self):
        ...
        self.assertEqual(
            [
                ("SAMPLE_ID", "00:00:00", "00:00:50", "1200", "0"),
...
class GetTripTimeTestCase(util.TestCase):
    ...
    def setUp(self):
        ...
        schedule = transitfeed.Schedule(problem_reporter=problems)
        self.schedule = schedule
        schedule.AddAgency("Agency", "http://iflyagency.com", "America/Los_Angeles")
        service_period = schedule.GetDefaultServicePeriod()
        service_period.SetDateHasService("20070101")
        ...
        self.trip1.AddStopTime(
            self.stop1, schedule=schedule, departure_secs=100, arrival_secs=100
        )
        ...
        self.trip1.AddStopTime(self.stop2, schedule=schedule)
        self.trip1.AddStopTime(
            self.stop4, schedule=schedule, departure_secs=400, arrival_secs=400
        )

        self.trip2 = self.route1.AddTrip(schedule, "trip 2", trip_id="trip2")
        self.trip2.AddStopTime(
            self.stop2, schedule=schedule, departure_secs=500, arrival_secs=500
        )
        self.trip2.AddStopTime(
            self.stop3, schedule=schedule, departure_secs=600, arrival_secs=600
        )
        self.trip2.AddStopTime(
            self.stop4, schedule=schedule, departure_secs=700, arrival_secs=700
        )
        self.trip2.AddStopTime(
            self.stop3, schedule=schedule, departure_secs=800, arrival_secs=800
        )
    ...
    def testGetTimeInterpolatedStops(self):
        ...
        self.assertEqual(
            ("140.01,0", "140.02,0", "140.03,0", "140.02,0", "140.04,0"),
    ...
    def testGetTimeInterpolatedStopsUntimedStart(self):
        ...
        self.trip3.AddStopTime(
            self.stop2, schedule=self.schedule, departure_secs=500, arrival_secs=500
        )
        self.assertRaises(ValueError, self.trip3.GetTimeInterpolatedStops)

    def testGetTimeInterpolatedStopsSingleStopTime(self):
        self.trip3.AddStopTime(
            self.stop3, schedule=self.schedule, departure_secs=500, arrival_secs=500
        )
    ...
    def testGetStopTimeTrips(self):
        ...
        self.assertEqual(
            ("trip1", "trip2", "trip2"), tuple([ti[0].trip_id for ti in trip_index])
        )
    ...
    def testGetTrips(self):
        ...
        self.assertEqual(
            set([t.trip_id for t in self.stop5.GetTrips(self.schedule)]), set()
        )
...
class GetFrequencyTimesTestCase(util.TestCase):
    ...
    def setUp(self):
        ...
        self.trip1.AddStopTime(
            self.stop1, arrival_time="17:00:00", departure_time="17:01:00"
    ...
    def testGetFrequencyStartTimes(self):
        ...
        self.assertEqual(start_times, self.trip1.GetFrequencyStartTimes())
