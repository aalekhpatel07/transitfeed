...
class DuplicateScheduleIDTestCase(util.TestCase):
    def runTest(self):
        ...
        try:
            schedule.Load(util.DataPath("duplicate_schedule_id"), extra_validation=True)
...
class OverlappingBlockTripsTestCase(util.TestCase):
    ...
    def setUp(self):
        ...
        self.stop2 = schedule.AddStop(
            lng=-116.76218, lat=36.905697, name="E Main St / S Irving St", stop_id="S2"
        )
...
class DuplicateTripTestCase(util.ValidationTestCase):
    def runTest(self):
        ...
        agency = transitfeed.Agency(
            "Demo agency", "http://google.com", "America/Los_Angeles", "agency1"
        )
...
class StopBelongsToBothSubwayAndBusTestCase(util.ValidationTestCase):
    def runTest(self):
        ...
        schedule.AddAgency("Demo Agency", "http://example.com", "America/Los_Angeles")
...
class ScheduleStartAndExpirationDatesTestCase(util.MemoryZipTestCase):
    ...
    one_week_ago = time.strftime(date_format, time.localtime(now - 7 * seconds_per_day))
    one_week = time.strftime(date_format, time.localtime(now + 7 * seconds_per_day))
    two_weeks = time.strftime(date_format, time.localtime(now + 14 * seconds_per_day))
    two_months = time.strftime(date_format, time.localtime(now + 60 * seconds_per_day))
...
class DuplicateStopValidationTestCase(util.ValidationTestCase):
    def runTest(self):
        ...
        schedule.AddAgency("Sample Agency", "http://example.com", "America/Los_Angeles")
...
class AgencyIDValidationTestCase(util.TestCase):
    def runTest(self):
        ...
        schedule.AddAgency(
            "Test Agency", "http://example.com", "America/Los_Angeles", "TEST_AGENCY"
        )
        schedule.AddRouteObject(route)
        ...
        schedule.AddAgency("Test Agency 3", "http://example.com", "America/Los_Angeles")
        schedule.AddRouteObject(route)
...
class DefaultAgencyTestCase(util.TestCase):
    ...
    def test_NewDefaultAgency(self):
        ...
        agency3 = schedule.NewDefaultAgency(
            agency_id="agency3", agency_name="Agency 3", agency_url="http://goagency"
        )
...
class ServiceGapsTestCase(util.MemoryZipTestCase):
    ...
    def testServiceGapBeforeTodayIsDiscovered(self):
        self.schedule.Validate(today=date(2009, 7, 17), service_gap_interval=13)
    ...
    def testNoServiceGapBeforeTodayIfTodayHasService(self):
        self.schedule.Validate(today=date(2009, 7, 18), service_gap_interval=13)
    ...
    def testNoServiceGapBeforeTodayIfTheFeedStartsToday(self):
        ...
        exception = self.accumulator.PopException("TooManyDaysWithoutService")
        self.assertEqual(date(2009, 6, 11), exception.first_day_without_service)
        self.assertEqual(date(2009, 7, 17), exception.last_day_without_service)
    ...
    def testGapAtTheEndOfTheOneYearPeriodIsDiscovered(self):
        self.schedule.Validate(today=date(2009, 6, 22), service_gap_interval=13)
    ...
    def testCurrentServiceGapIsDiscovered(self):
        self.schedule.Validate(today=date(2009, 6, 30), service_gap_interval=13)
        exception = self.accumulator.PopException("TooManyDaysWithoutService")
        self.assertEqual(date(2009, 6, 18), exception.first_day_without_service)
        self.assertEqual(date(2009, 7, 17), exception.last_day_without_service)
    ...
    def AssertCommonExceptions(self, last_exception_date):
        exception = self.accumulator.PopException("TooManyDaysWithoutService")
        self.assertEqual(date(2009, 8, 10), exception.first_day_without_service)
        self.assertEqual(date(2009, 8, 22), exception.last_day_without_service)

        exception = self.accumulator.PopException("TooManyDaysWithoutService")
        self.assertEqual(date(2009, 12, 28), exception.first_day_without_service)
        self.assertEqual(date(2010, 1, 15), exception.last_day_without_service)

        if last_exception_date is not None:
            exception = self.accumulator.PopException("TooManyDaysWithoutService")
            self.assertEqual(date(2010, 6, 7), exception.first_day_without_service)
            self.assertEqual(last_exception_date, exception.last_day_without_service)
