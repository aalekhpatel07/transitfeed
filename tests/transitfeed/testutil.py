...
class ColorLuminanceTestCase(test_util.TestCase):
    def runTest(self):
        self.assertEqual(
            util.ColorLuminance("000000"), 0, "ColorLuminance('000000') should be zero"
        )
        self.assertEqual(
            util.ColorLuminance("FFFFFF"), 255, "ColorLuminance('FFFFFF') should be 255"
        )
        RGBmsg = (
...
class MockURLOpen:
    ...
    def mockedPageNotFound(self, request):
        raise HTTPError(
            request.get_full_url(), 404, "Not Found", request.header_items(), None
        )
