...
class EndOfLineCheckerTestCase(util.TestCase):
    ...
    def testEmbeddedCr(self):
        f = transitfeed.EndOfLineChecker(
            StringIO("line1\rline1b"), "<StringIO>", self.problems
        )
        self.RunEndOfLineChecker(f)
        e = self.accumulator.PopException("OtherProblem")
        self.assertEqual(e.file_name, "<StringIO>")
    ...
    def testEmbeddedUtf8NextLine(self):
        f = transitfeed.EndOfLineChecker(
            StringIO("line1b\xc2\x85"), "<StringIO>", self.problems
        )
        self.RunEndOfLineChecker(f)
        e = self.accumulator.PopException("OtherProblem")
        self.assertEqual(e.file_name, "<StringIO>")
        self.assertEqual(e.row_num, 1)
        self.assertEqual(
            e.FormatProblem(), "Line contains Unicode NEXT LINE SEPARATOR U+0085"
        )
        self.accumulator.AssertNoMoreExceptions()

    def testEndOfLineMix(self):
        f = transitfeed.EndOfLineChecker(
            StringIO("line1\nline2\r\nline3\nline4"), "<StringIO>", self.problems
        )
        self.RunEndOfLineChecker(f)
        e = self.accumulator.PopException("OtherProblem")
        self.assertEqual(e.file_name, "<StringIO>")
    ...
    def testLoad(self):
        loader = transitfeed.Loader(
            util.DataPath("bad_eol.zip"), problems=self.problems, extra_validation=True
        )
        loader.Load()
        ...
        self.assertEqual(e.row_num, 1)
        self.assertTrue(re.search(r"contains ASCII Form Feed", e.FormatProblem()))
        # TODO(Tom): avoid this duplicate error for the same issue
        e = self.accumulator.PopException("CsvSyntax")
        self.assertEqual(e.row_num, 1)
        self.assertTrue(
            re.search(
                r"header row should not contain any space char", e.FormatProblem()
            )
        )
...
class ZipCompressionTestCase(util.MemoryZipTestCase):
    def runTest(self):
        ...
        self.assertFalse(
            ...
            "Orginial size: %d  recompressed: %d" % (write_size, recompressedzip_size),
        )
...
class LoadUTF16TestCase(util.TestCase):
    def runTest(self):
        ...
        loader = transitfeed.Loader(
            util.DataPath("utf16"), problems=problem_reporter, extra_validation=True
        )
        try:
            loader.Load()
...
class CsvDictTestCase(util.TestCase):
    ...
    def testHeaderOnly(self):
        ...
        results = list(
            self.loader._ReadCsvDict("test.txt", ["test_id", "test_name"], [], [])
        )
        self.assertEqual([], results)
        self.accumulator.AssertNoMoreExceptions()
    ...
    def testQuotedFieldWithSpaces(self):
        ...
        self.assertEqual(
            [
                (
                    {"test_id": "id1", "test_name": "my name", "test_size": "234"},
                    2,
    ...
    def testQuotedFieldWithCommas(self):
        ...
        results = list(
            self.loader._ReadCsvDict("test.txt", ["id", "name1", "name2"], [], [])
        )
        self.assertEqual(
            [
                (
                    {"id": "1", "name1": "brown, tom", "name2": 'brown, "tom"'},
                    2,
                    ["id", "name1", "name2"],
                    ["1", "brown, tom", 'brown, "tom"'],
                )
    ...
    def testDeprecatedColumnWithoutNewColumn(self):
        ...
        results = list(
            self.loader._ReadCsvDict(
                "test.txt", ["test_id", "test_new"], ["test_id"], [("test_old", None)]
            )
        )
        self.assertEqual([], results)
        e = self.accumulator.PopException("DeprecatedColumn")
...
class BasicParsingTestCase(util.TestCase):
    ...
    def assertLoadedStopTimesCorrectly(self, schedule):
        ...
        self.assertEqual(
            "to airport", schedule.GetTrip("STBA").GetStopTimes()[0].stop_headsign
        )
