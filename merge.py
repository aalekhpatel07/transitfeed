...
class MergeProblemWithContext(transitfeed.ExceptionWithContext):
    ...
    def __init__(self, dataset_merger, problem_type=transitfeed.TYPE_WARNING, **kwargs):
...
class FareRulesBroken(MergeProblemWithContext):
    ERROR_TEXT = "The feed merger is currently unable to handle fare rules " "properly."
...
class MergeProblemReporter(transitfeed.ProblemReporter):
    ...
    def SameIdButNotMerged(self, dataset, entity_id, reason):
        self.AddToAccumulator(SameIdButNotMerged(dataset, id=entity_id, reason=reason))
...
class HTMLProblemAccumulator(transitfeed.ProblemAccumulatorInterface):
    ...
    def _GenerateNotices(self):
        ...
        for e in self._notices:
            ...
            items.append(
                '<li class="notice">%s</li>' % e.FormatProblem(d).replace("\n", "<br>")
            )
        if items:
    ...
    def WriteOutput(
        self, output_file, feed_merger, old_feed_path, new_feed_path, merged_feed_path
    ):
...
class DataSetMerger(object):
    ...
    def _MergeSameAgency(self, a_agency_id, b_agency_id):
        ...
        """
        a_agency_id = (
            a_agency_id or self.feed_merger.a_schedule.GetDefaultAgency().agency_id
        )
        b_agency_id = (
            b_agency_id or self.feed_merger.b_schedule.GetDefaultAgency().agency_id
        )
    ...
    def _SchemedMerge(self, scheme, a, b):
        ...
        for attr, merger in list(scheme.items()):
            ...
            try:
                ...
                raise MergeError(
                    "Attribute '%s' could not be merged: %s." % (attr, merge_error)
                )
    ...
    def _ReportSameIdButNotMerged(self, entity_id, reason):
        ...
        """
        self.feed_merger.problem_reporter.SameIdButNotMerged(self, entity_id, reason)
...
class ServicePeriodMerger(DataSetMerger):
    ...
    def _Migrate(self, original_service_period, schedule, newid):
        ...
        migrated_service_period.day_of_week = list(original_service_period.day_of_week)
    ...
    def _Add(self, a, b, migrated_service_period):
        ...
        self.feed_merger.merged_schedule.AddServicePeriodObject(migrated_service_period)
...
class FareMerger(DataSetMerger):
    ...
    def _Migrate(self, original_fare, schedule, newid):
        ...
        if newid:
            migrated_fare.fare_id = self.feed_merger.GenerateId(original_fare.fare_id)
...
class ShapeMerger(DataSetMerger):
    ...
    def _MergeEntities(self, a, b):
        ...
        distance = max(
            ...
            ApproximateDistanceBetweenPoints(a.points[-1][:2], b.points[-1][:2]),
        )
...
class TripMerger(DataSetMerger):
    ...
    def _Migrate(self, original_trip, schedule, newid):
        ...
        # with an existing id.
        if newid:
            migrated_trip.trip_id = self.feed_merger.GenerateId(original_trip.trip_id)
            migrated_trip.original_trip_id = original_trip.trip_id
        # Need to add trip to schedule before copying stoptimes
        self.feed_merger.merged_schedule.AddTripObject(migrated_trip, validate=False)
        ...
        original_service_period = schedule.GetServicePeriod(original_trip.service_id)
        ...
        for (schedule, merge_map, zone_map) in (
            ...
            for fare in schedule.GetFareAttributeList():
                for fare_rule in fare.GetFareRuleList():
                    ...
                    route_id = (
                        fare_rule.route_id
                        and merge_map[schedule.GetRoute(fare_rule.route_id)].route_id
                    )
                    origin_id = fare_rule.origin_id and zone_map[fare_rule.origin_id]
                    destination_id = (
                        fare_rule.destination_id and zone_map[fare_rule.destination_id]
                    )
                    contains_id = (
                        fare_rule.contains_id and zone_map[fare_rule.contains_id]
                    )
                    rules.add(
...
class FeedMerger(object):
    ...
    def __init__(self, a_schedule, b_schedule, merged_schedule, problem_reporter):
...
"""
    ...
    accumulator.WriteOutput(
        output_file, feed_merger, old_feed_path, new_feed_path, merged_feed_path
    )
    output_file.close()
