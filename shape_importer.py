...
def GetMatchingShape(pattern_poly, trip, matches, max_distance, verbosity=0):
    ...
    scores = [(pattern_poly.GreedyPolyMatchDist(match), match) for match in matches]
...
def main(key_cols):
    ...
    for route in schedule.GetRouteList():
        ...
        for pattern_id, trips in patterns.items():
            ...
            poly_points = [
                shapelib.Point.FromLatLng(p.stop_lat, p.stop_lon) for p in pattern
            ]
            if verbosity >= 2:
                print("\npattern %d, %d points:" % (pattern_id, len(poly_points)))
                for i, (stop, point) in enumerate(zip(pattern, poly_points)):
                    print(
                        "Stop %d '%s': %s" % (i + 1, stop.stop_name, point.ToLatLng())
                    )
            ...
            if shape_match:
                ...
                shape_match = shapelib.Poly(
                    points=shape_match.GetPoints(), name="shape_%d" % shape_count
                )
                for trip in trips:
                    try:
