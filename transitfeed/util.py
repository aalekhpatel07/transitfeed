...
LATEST_RELEASE_VERSION_URL = (
    "https://raw.githubusercontent.com/wiki/google/transitfeed/LatestReleaseVersion.md"
)
...
def CheckVersion(problems, latest_version=None):
    ...
    if not latest_version:
        ...
        try:
            ...
            description = (
                "During the new-version check, we failed to reach "
                "transitfeed server: Reason: %s [%s]." % (e.reason, e.code)
            )
    ...
    if not latest_version:
        description = (
            "During the new-version check, we had trouble parsing the "
...
def ValidateAndReturnIntValue(
    value, allowed_values, default, allow_empty, column_name=None, problems=None
):
    """
    ...
    try:
        ...
        if int_value not in allowed_values:
            if problems and column_name:
                problems.InvalidValue(column_name, value, type=errors.TYPE_WARNING)
        return int_value
...
def ApproximateDistanceBetweenStops(stop1, stop2):
    ...
    Earth is a sphere."""
    if (
        stop1.stop_lat is None
        or stop1.stop_lon is None
        or stop2.stop_lat is None
        or stop2.stop_lon is None
    ):
        return None
    return ApproximateDistance(
        stop1.stop_lat, stop1.stop_lon, stop2.stop_lat, stop2.stop_lon
    )
        ...
        for seq, name in list(INVALID_LINE_SEPARATOR_UTF8.items()):
            if next_line_contents.find(seq) != -1:
                self._problems.OtherProblem(
                    "Line contains %s" % name, context=(self._name, self._line_number)
                )
