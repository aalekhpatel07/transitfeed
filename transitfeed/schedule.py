...
class Schedule(object):
        ...
        def connector(db_file):
            if native_sqlite:
                ...
                return sqlite.connect(
                    "jdbc:sqlite:%s" % db_file, "", "", "org.sqlite.JDBC"
                )
    ...
    def GetStopBoundingBox(self):
        return (
            min(s.stop_lat for s in list(self.stops.values())),
    ...
    def AddServicePeriodObject(
        self, service_period, problem_reporter=None, validate=True
    ):
        if not problem_reporter:
            problem_reporter = self.problem_reporter
    ...
    def GetDateRangeWithOrigins(self):
        ...
        minreason = (
            period_list[minindex].HasDateExceptionOn(minvalue)
            and "earliest service exception date in calendar_dates.txt"
            or "earliest service date in calendar.txt"
        )

        maxreason = (
            period_list[maxindex].HasDateExceptionOn(maxvalue)
            and "last service exception date in calendar_dates.txt"
            or "last service date in calendar.txt"
        )
    ...
    def AddRoute(self, short_name, long_name, route_type, route_id=None):
        ...
        route = self._gtfs_factory.Route(
        ...
        if route.agency_id not in self._agencies:
            if not route.agency_id and len(self._agencies) == 1:
                ...
                problem_reporter.InvalidValue(
                    "agency_id", route.agency_id, "Route uses an unknown agency_id."
                )
                return
    ...
    def AddTransferObject(self, transfer, problem_reporter=None):
        ...
        if transfer_id in self._transfers:
            self.problem_reporter.DuplicateID(
                self._gtfs_factory.Transfer._ID_COLUMNS,
                transfer_id,
    ...
    def Load(self, feed_path, extra_validation=False):
        loader = self._gtfs_factory.Loader(
            feed_path,
            self,
    ...
    def WriteGoogleTransitFeed(self, file):
        ...
        if has_data:
            ...
            self._WriteArchiveString(
                archive, "calendar_dates.txt", calendar_dates_string
            )
    ...
    def ValidateFeedStartAndExpirationDates(
        self,
        problems,
        first_date,
        last_date,
        ...
        if last_date < warning_cutoff:
            problems.ExpirationDate(
                time.mktime(last_date.timetuple()), last_date_origin
            )

        if first_date > today:
            problems.FutureService(
                time.mktime(first_date.timetuple()), first_date_origin
            )

    def ValidateServiceGaps(
        self, problems, validation_start_date, validation_end_date, service_gap_interval
    ):
        ...
        departures = self.GenerateDateTripsDeparturesList(
        ...
        for day_date, day_trips, _ in departures:
            if day_trips == 0:
                ...
                if consecutive_days_without_service >= service_gap_interval:
                    problems.TooManyDaysWithoutService(
    ...
    def ValidateServiceRangeAndExceptions(self, problems, today, service_gap_interval):
        ...
        (
            ...
            start_date_origin,
            end_date_origin,
        ) = self.GetDateRangeWithOrigins()
        if not end_date or not start_date:
            problems.OtherProblem(
                "This feed has no effective service dates!",
                type=problems_module.TYPE_WARNING,
            )
        else:
            try:
                last_service_day = datetime.datetime(
                    *(time.strptime(end_date, "%Y%m%d")[0:6])
                ).date()
                first_service_day = datetime.datetime(
                    *(time.strptime(start_date, "%Y%m%d")[0:6])
                ).date()
                ...
                self.ValidateServiceExceptions(
                    problems, first_service_day, last_service_day
                )
                self.ValidateFeedStartAndExpirationDates(
                    problems,
                    first_service_day,
                    last_service_day,
                    start_date_origin,
                ...
                if service_gap_interval is not None:
                    ...
                    self.ValidateServiceGaps(
                        problems,
                        max(first_service_day, today - service_gap_timedelta),
                        min(last_service_day, today + one_year),
    ...
    def ValidateStops(self, problems, validate_children):
        ...
        for stop in list(self.stops.values()):
            ...
            cursor = self._connection.cursor()
            cursor.execute(
                "SELECT count(*) FROM stop_times WHERE stop_id=? LIMIT 1",
                (stop.stop_id,),
            )
            ...
            if stop.location_type != 1 and stop.parent_station:
                if stop.parent_station not in self.stops:
                    ...
                    distance = util.ApproximateDistanceBetweenStops(
                        stop, parent_station
                    )
                    if distance is not None:
                        if (
                            distance
                            > problems_module.MAX_DISTANCE_BETWEEN_STOP_AND_PARENT_STATION_ERROR
                        ):
                            ...
                            distance
                            > problems_module.MAX_DISTANCE_BETWEEN_STOP_AND_PARENT_STATION_WARNING
                        ):
                            problems.StopTooFarFromParentStation(
                                stop.stop_id,
                                stop.stop_name,
                ...
                if distance is not None and distance < 2:
                    ...
                    if stop.location_type == 0 and other_stop.location_type == 0:
                        problems.StopsTooClose(
                            ...
                            util.EncodeUnicode(other_stop.stop_id),
                            distance,
                        )
                    ...
                    elif stop.location_type in (0, 1) and other_stop.location_type in (
                        0,
                        1,
                    ):
                        ...
                        if this_stop.parent_station != this_station.stop_id:
                            problems.DifferentStationTooClose(
                                ...
                                util.EncodeUnicode(this_station.stop_id),
                                distance,
                            )
                index += 1
    ...
    def ValidateTrips(self, problems):
        ...
        for trip in sorted(self.trips.values()):
            ...
            for index, st in enumerate(stop_times):
                ...
                if (
                    route_type == self._gtfs_factory.Route._ROUTE_TYPE_NAMES["Subway"]
                    or route_type == self._gtfs_factory.Route._ROUTE_TYPE_NAMES["Bus"]
                ):
                    if stop_id not in stop_types:
                        stop_types[stop_id] = [trip.route_id, route_type, 0]
                    elif (
                        stop_types[stop_id][1] != route_type
                        and stop_types[stop_id][2] == 0
                    ):
                        stop_types[stop_id][2] = 1
                        if (
                            stop_types[stop_id][1]
                        ...
                        problems.StopWithMultipleRouteTypes(
                            st.stop.stop_name, stop_id, subway_route_id, bus_route_id
                        )
            ...
            if self._check_duplicate_trips:
                ...
                if key not in trips:
                    ...
                    problems.DuplicateTrip(
                        trips[key][1], trips[key][0], trip.trip_id, trip.route_id
                    )
    ...
    def ValidateStopTimesForTrip(self, problems, trip, stop_times):
        ...
        def CheckSameTimeCount():
            ...
            if (
                prev_departure_secs != -1
                and consecutive_stop_times_with_fully_specified_same_time > 5
            ):
                problems.TooManyConsecutiveStopTimesWithSameTime(
                    trip.trip_id,
        ...
        for index, st in enumerate(stop_times):
            if st.arrival_secs is None or st.departure_secs is None:
                ...
                continue
            if (
                prev_departure_secs == st.arrival_secs
                and st.arrival_secs == st.departure_secs
            ):
                consecutive_stop_times_with_potentially_same_time += 1
                consecutive_stop_times_with_fully_specified_same_time = (
            ...
            for xi in range(len(trip_intervals)):
                ...
                for xj in range(xi + 1, len(trip_intervals)):
                    ...
                    if trip_a.service_id == trip_b.service_id:
                        problems.OverlappingTripsInSameBlock(
                            trip_a.trip_id, trip_b.trip_id, block_id
                        )
                    else:
                        ...
                        service_id_pair_key = tuple(
                            sorted([trip_a.service_id, trip_b.service_id])
                        )
        ...
        if len(self._agencies) > 1:
            for agency in list(self._agencies.values()):
                if util.IsEmpty(agency.agency_id):
                    problems.OtherProblem(
                        'Agency "%s" does not have an ID. '
                        "This is only allowed if a single agency is defined, "
                        "whereas there are %d in total."
                        % (agency.agency_name, len(self._agencies))
                    )
    ...
    def ValidateRouteAgencyId(self, problems):
        ...
        for route in list(self.routes.values()):
            if (
                not util.IsEmpty(route.agency_id)
                and not route.agency_id in self._agencies
            ):
                problems.InvalidAgencyID(
                    "agency_id", route.agency_id, "route", route.route_id
                )
    ...
    def ValidateUnusedShapes(self, problems):
        ...
        if unused_shape_ids:
            problems.OtherProblem(
                "The shapes with the following shape_ids aren't "
                "used by any trips: %s" % ", ".join(unused_shape_ids),
                type=problems_module.TYPE_WARNING,
            )
