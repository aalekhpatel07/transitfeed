...
class Trip(GtfsObjectBase):
    ...
    def GetTimeStops(self):
        ...
        return [
            (st.arrival_secs, st.departure_secs, st.stop) for st in self.GetStopTimes()
        ]
        ...
        for i, st in enumerate(stoptimes):
            if st.GetTimeSecs() != None:
                ...
                distance_traveled_between_timepoints += (
                    util.ApproximateDistanceBetweenStops(stoptimes[i - 1].stop, st.stop)
                )
                distance_percent = (
                    distance_traveled_between_timepoints / distance_between_timepoints
                )
    ...
    def GetEndTime(self, problems=problems_module.default_problem_reporter):
        ...
        if departure_secs != None:
            ...
            problems.InvalidValue(
                "arrival_time",
                "",
                "The last stop_time in trip %s is missing " "times." % self.trip_id,
            )
    ...
    def ValidateDirectionId(self, problems):
        if (
            ...
            problems.InvalidValue(
                "direction_id", self.direction_id, 'direction_id must be "0" or "1"'
            )
    ...
    def _CheckSpeed(
        self, prev_stop, next_stop, depart_time, arrive_time, max_speed, problems
    ):
