...
class GtfsLocation(object):
    ...
    def __init__(self, row, gtfs_reader):
        ...
        self.location_type = LocationType(int(row.get("location_type", 0) or 0))
    ...
    def has_pathways(self):
        return len(self.incoming_pathways) > 0 or len(self.outgoing_pathways) > 0

    def self_or_children_have_pathways(self):
        if self.has_pathways():
            return True
        for child_id in self.children:
            if self._reader.get_location(child_id).self_or_children_have_pathways():
                return True
        return False
...
def requires_platform_cluster(location):
    return location.location_type == LocationType.platform and location.has_children()
