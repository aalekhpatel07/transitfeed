...
SPECIAL_CITIES = {"Affoltern a. A.": "Affoltern am Albis", "Wangen b. D.": "Wangen"}
...
class DivaImporter:
    ...
    def DemangleName(self, name):
        ...
        for abbrev, expanded in [("str.", "strasse"), ("Schiffst.", "Schiffstation")]:
    ...
    def Import(self, inpath):
        ...
        self.ImportStations(
            read("rec_ort.mdv"), read("bedienendeLinien_google.csv", "ORT_NR;LI_NR;;;;")
        )
        self.ImportRoutes(read("rec_lin_ber.mdv"))
        self.ImportPatterns(read("lid_verlauf.mdv"))
        self.ImportServices(read("tagesart_merkmal.mdv"), read("firmenkalender.mdv"))
    ...
    def ImportStations(self, station_file, adv_file):
        ...
        for id, name, x, y, uic_code in ReadCSV(
            station_file,
            ["ORT_NR", "ORT_NAME", "ORT_POS_X", "ORT_POS_Y", "ORT_NR_NATIONAL"],
        ):
            station = Station()
            station.id = id
            ...
            station.country = "CH"
            station.url = "http://fahrplan.zvv.ch/?to.0=" + urllib.parse.quote(
        ...
        for station_id, line_id in ReadCSV(adv_file, ["ORT_NR", "LI_NR"]):
            if station_id in self.stations:
                ...
                print(
                    "Warning, advertised lines file references "
    ...
    def ImportRoutes(self, s):
        ...
        for line_id, name in ReadCSV(s, ["LI_NR", "LINIEN_BEZ_DRUCK"]):
            route = Route()
            route.id = line_id
            route.name = name
            route.color = "FFFFFF"
    ...
    def ImportPatterns(self, s):
        ...
        for line, strli, direction, seq, station_id in ReadCSV(
            s, ["LI_NR", "STR_LI_VAR", "LI_RI_NR", "LI_LFD_NR", "ORT_NR"]
        ):
    ...
    def ImportBoarding(self, drop_off_file):
        ...
        for trip_id, seq, code in ReadCSV(
            drop_off_file, ["FRT_FID", "LI_LFD_NR", "BEDVERB_CODE"]
        ):
            ...
            if code == "A":
                ...
                raise ValueError(
                    "Unexpected code in bedverb.mdv; "
    ...
    def ImportServices(self, daytype_file, days_file):
        ...
        for schedule, daytype, date in ReadCSV(
            days_file, ["FPL_KUERZEL", "TAGESART_NR", "BETRIEBSTAG"]
        ):
        ...
        for daytype, service_id in ReadCSV(
            daytype_file, ["TAGESART_NR", "TAGESMERKMAL_NR"]
        ):
    ...
    def ImportTrafficRestrictions(self, restrictions_file):
        ...
        for schedule, id, bitmask, start_date, end_date in ReadCSV(
            restrictions_file,
            ...
            for i in range(MonthNr(end_date) - MonthNr(start_date) + 1):
                ...
                for d in range(32):
                    if 1 << d & mask:
                        year = (
                            int(start_date[0:4]) + ((int(start_date[4:6]) + i - 1)) / 12
                        )
                        month = ((int(start_date[4:6]) + i - 1) % 12) + 1
                        day = d + 1
                        cur_date = (
                            str(year) + ("0" + str(month))[-2:] + ("0" + str(day))[-2:]
                        )
    ...
    def ImportStopTimes(self, stoptimes_file):
        ...
        for line, strli, direction, seq, stoptime_id, drive_secs, wait_secs in ReadCSV(
            stoptimes_file,
            [
                "LI_NR",
                "STR_LI_VAR",
    ...
    def WriteStations(self, out):
        out.write(
            "stop_id,stop_uic_code,stop_name,stop_city,stop_country,"
    ...
    def WriteStopTimes(self, out):
        ...
        for (trip_id, trip) in trips:
            ...
            for seq in range(len(trip.stoptimes)):
                ...
                if (
                    not self._drop_unadvertised_lines
                    or trip.route.id in station.advertised_lines
                ):
                    WriteRow(
                        out,
                        [
