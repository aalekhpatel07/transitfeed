...
class KMLWriter(object):
    ...
    def _CreateLineStringForShape(self, parent, shape):
        ...
        """
        coordinate_list = [
            (longitude, latitude) for (latitude, longitude, distance) in shape.points
        ]
    ...
    def _CreateStopsFolder(self, schedule, doc):
        ...
        for stop in stops:
            ...
            if (
                ...
                and stop.location_type != transitfeed.Stop.LOCATION_TYPE_STATION
    ...
    def _CreateStopPlacemark(self, stop_folder, stop, style_id):
        ...
        if stop.stop_url:
            desc_items.append(
                'Stop info page: <a href="%s">%s</a>' % (stop.stop_url, stop.stop_url)
            )
        description = "<br/>".join(desc_items) or None
        placemark = self._CreatePlacemark(
            stop_folder, stop.stop_name, description=description, style_id=style_id
        )
        point = ET.SubElement(placemark, "Point")
        coordinates = ET.SubElement(point, "coordinates")
        coordinates.text = "%.6f,%.6f" % (stop.stop_lon, stop.stop_lat)

    def _CreateRoutePatternsFolder(self, parent, route, style_id=None, visible=True):
        ...
        for n, trips in enumerate(pattern_trips):
            ...
            coordinates = [
                (stop.stop_lon, stop.stop_lat) for stop in trips[0].GetPattern()
            ]
            self._CreateLineString(placemark, coordinates)
        return folder
