...
class ScheduleRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def handle_GET_home(self):
        ...
        agency = ", ".join(a.agency_name for a in schedule.GetAgencyList()).encode(
            "utf-8"
        )
    ...
    def handle_json_GET_routepatterns(self, params):
        ...
        for pattern_id, trips in list(pattern_id_trip_dict.items()):
            ...
            name = "%s to %s, %d stops" % (
                time_stops[0][2].stop_name,
                time_stops[-1][2].stop_name,
                len(time_stops),
            )
    ...
    def handle_json_GET_boundboxstops(self, params):
        ...
        limit = int(params.get("limit"))
        stops = schedule.GetStopsInBoundingBox(
            north=n, east=e, south=s, west=w, n=limit
        )
        return [StopToTuple(s) for s in stops]
    ...
    def handle_json_GET_stopsearch(self, params):
        ...
        for s in schedule.GetStopList():
            if (
                s.stop_id.lower().find(query) != -1
                or s.stop_name.lower().find(query) != -1
            ):
    ...
    def handle_GET_ttablegraph(self, params):
        ...
        if pattern_id not in pattern_id_trip_dict:
            print(
                "no pattern %s found in %s"
                % (pattern_id, list(pattern_id_trip_dict.keys()))
            )
            self.send_error(404)
            return
...
"""
    ...
    parser.add_option("--port", dest="port", type="int", help="port on which to listen")
    parser.add_option(
        "--file_dir", dest="file_dir", help="directory containing static files"
    )
    parser.add_option(
        "-n",
        "--noprompt",
    ...
    server = StoppableHTTPServer(
        server_address=("", options.port), RequestHandlerClass=RequestHandlerClass
    )
